...
export function DragErrorHandler({
  ...
  const addError = useCallback(
    (error: Omit<DragError, "id" | "timestamp">) => {
  ...
  const handleRetry = useCallback(
    async (error: DragError) => {
      ...
      if (currentRetries >= maxRetries) {
        ...
        setErrors((prev) =>
          prev.map((e) =>
            e.id === error.id ? { ...e, isRecoverable: false, message: `${e.message} (Max retries exceeded)` } : e
          )
        );
        return false;
      }
      ...
      // Update retry count
      setErrors((prev) => prev.map((e) => (e.id === error.id ? { ...e, retryCount: currentRetries + 1 } : e)));
 ...
 */
function ErrorMessage({ error, onRetry, onDismiss }: { error: DragError; onRetry: () => void; onDismiss: () => void }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1">
        <AlertTitle
          className={cn(severity === "warning" && "text-yellow-800", severity === "destructive" && "text-red-800")}
        >
...
export function useDragErrorHandler() {
  const [errorHandler, setErrorHandler] = useState<React.ComponentType<DragErrorHandlerProps> | null>(null);

  const handleError = useCallback(
    (type: DragErrorType, message: string, details?: string, operation?: DragOperation, isRecoverable = true) => {
  ...
  const handleValidationError = useCallback(
    (message: string, operation?: DragOperation) => {
      handleError(
        "VALIDATION_ERROR",
        message,
        undefined,
  ...
  const handleConflictError = useCallback(
    (conflicts: string[], operation?: DragOperation) => {
      handleError(
        "CONFLICT_ERROR",
  ...
  const handleTimeoutError = useCallback(
    (operation?: DragOperation) => {
      handleError("TIMEOUT_ERROR", "Operation timed out", "The server took too long to respond", operation);
    },
    [handleError]
  );
...
export class EdgeCaseValidator {
  ...
  static validateRapidInteractions(lastInteractionTime: number, minDelay = 100): { isValid: boolean; reason?: string } {
